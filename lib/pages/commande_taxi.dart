...
class CommandeTaxi extends StatefulWidget {
  const CommandeTaxi({super.key});

  @override
  State<CommandeTaxi> createState() => CommandeTaxiState();
}

class CommandeTaxiState extends State<CommandeTaxi> {
  int step_index = 0;
  dynamic lieux_selected = {};
  bool boissons_expanded = false;
  bool accompagnement_expanded = false;
  FormGroup taxi_transport_form = FormGroup({
        ...
        child: Column(
          ...
          children: [
            ...
            ReactiveTextField(
              ...
              onTap: (control) {
                launch_map_for_result("lieu de départ", "lieu_recup", null);
              },
              decoration: Utils.get_default_input_decoration_normal(
                  taxi_transport_form.control('lieu_recup'),
                  false,
                  'Lieu de départ',
            ...
            ReactiveTextField(
              ...
              decoration: Utils.get_default_input_decoration_normal(
                  taxi_transport_form.control('lieu_dest'),
                  false,
                  'Lieu de destination',
                  {"icon": Icons.my_location, "size": 24.sp},
                  Colors.transparent,
            ...
            IntlPhoneField(
              decoration: Utils.get_default_input_decoration_normal(
                  taxi_transport_form.control('contact_dest'),
                  true,
                  'Contact à appeler',
                  null,
                  Colors.transparent,
            ...
            ReactiveTextField(
              ...
              decoration: Utils.get_default_input_decoration_multi_lines(
                  taxi_transport_form.control('message'),
                  false,
                        ...
                        children: [
                          ...
                          Column(
                            ...
                            children: [
                              ...
                              MediumBoldText(
                                  text:
                                      "${Utils.nf(tarif["cout_livraison"])} FCFA"),
                            ],
                          )
                        ],
                      ),
  ...
  void store_commande(mode_paiement, commande_info) {
    ...
    var store_commande_obj = {
      ...
      "type_commande_id": Utils.TYPE_COMMANNDES["taxi"],
      "tauxPromotion": commande_info["tauxPromotion"],
      "client_id": Utils.client["client_id"],
      "coutKm": commande_info["coutKm"],
      "restaurant_id": null,
      "lieuLivraison": taxi_transport_form.rawValue["lieu_dest"],
      "longitudeLivraison": lieux_selected["lieu_dest"]["longitude"],
      "latitudeLivraison": lieux_selected["lieu_dest"]["latitude"],
      "lieuRecuperation": taxi_transport_form.rawValue["lieu_recup"],
      "longitudeRecuperation": lieux_selected["lieu_recup"]["longitude"],
      "latitudeRecuperation": lieux_selected["lieu_recup"]["latitude"],
      "contactLivraison": taxi_transport_form.rawValue["contact_dest"],
      "contactRecuperation": taxi_transport_form.rawValue["contact_dest"],
      "messageAuLivreur": taxi_transport_form.rawValue["message"],
      ...
      http.get(get_route_url(start, end)).then((response) {
        ...
        if (response.statusCode == 200) {
          ...
          Utils.log(data["features"]?[0]?["properties"]?["segments"]?[0]
              ?["distance"]);
          /* taxi_transport_form.control("lieu_dest")?.patchValue(
            "${taxi_transport_form.rawValue["lieu_dest"]} - ${repas_livraison_distance / 1000}km"); */
          if (mounted) setState(() {});
  ...
  void show_repick_dialog(String end) {
    showDialog(
        ...
        builder: (context) {
          return AlertDialog(
            title:
                const MediumBoldText(text: "Emplacement géographique invalide"),
            content: SmallRegularText(
                text: "Veuillez selectionner un emplacement plus précis."),
            actions: <Widget>[
              TextButton(
                child: const SmallRegularText(text: 'Annuler'),
                onPressed: () {
                  Navigator.of(context).pop();
                },
              ),
              TextButton(
                child: const SmallBoldText(text: 'Rééessayer →'),
                onPressed: () {
                  Navigator.of(context).pop();
                      ...
                      steps: <Step>[
                        Step(
                          title: SmallTitreText(
                              text:
                                  step_index == 0 ? "Infos de transport" : ""),
                          content: get_step_1(),
                          ...
                          children: [
                            ...
                            if (step_index == 0)
                              AppButton(
                                ...
                                disabled: !taxi_transport_form.valid,
                                child_type: "text",
                                svg_image_size: "wx16",
                                text: "Tarification →",
                                text_size: "small",
